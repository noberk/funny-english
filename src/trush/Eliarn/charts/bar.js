let data =
    `
00:03
Zb Thank you so much, Chris. And it's truly a great honor to have the opportunity to come to this stage twice; I'm extremely grateful. I have been blown away by this conference, and I want to thank all of you for the many nice comments about what I had to say the other night. And I say that sincerely, partly because (Mock sob) I need that. 
...
function wordFrequency(arr) {
    ...
    for (let i = 0; i < a.length; i++) {
        result.push({ "word": a[i], "times": b[i] })
    }
    return result;

}
function sortByTimes() {
    let arr = wordFrequency(newArray);
    return arr.sort((a, b) => (a.times < b.times) ? 1 : -1);
}




const arr = sortByTimes();
function Draw(data, container) {
    let body = document.getElementById(container);

    data.forEach(item => {
        let div = createLine(item);
...
function createLine(item) {
    ...
    let times = createElement("strong");
    let timesWidth= (item.times / arr.length * 100*30).toString() + "px";
    console.log(timesWidth);
   times.style.width =  timesWidth;
   

    let vocab = createElement("span");
    outer.appendChild(times);
    outer.appendChild(vocab);

    times.innerHTML = item.times;
    //  wordSpace.innerHTML = word;
    vocab.innerHTML = item.word;
...
lab.innerText=`Total words of this article are ${newArray.length}. unique words count are ${arr.length}.` 